...
type GroceryStore = {
  ...
  isLoading: boolean;
  error: string | null;
  loadItems: () => Promise<void>; // gọi đến hàm mà có kết quả liền thì không cần trả về Promise, nhưng nếu có kết quả trả về sau một khoảng thời gian thì nên trả về Promise để dễ dàng xử lý kết quả sau khi hoàn thành
...
export const useGroceryStore = create<GroceryStore>((set, get) => ({
  ...
  loadItems: async () => {
    ...
    try {
      ...
      set({ error: "Something went wrong while loading items." });
    } finally {
      set({ isLoading: false });
    }
  },
  addItem: async (input) => {
    ...
    try {
      ...
      set({ error: "Something went wrong while adding the item." });
    }
  },
  updateQuantity: async (id, quantity) => {
    ...
    try {
      ...
      set({ error: "Something went wrong while updating the item quantity." });
    }
  },
